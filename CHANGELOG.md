...
## 2026-04-07 - P0 语音闭环与硬件协同主线完成

### 设备按住说话 + 电脑麦克风代采主线

- 明确当前语音主入口仍为 `按住设备说话`，实际采音改由 `电脑麦克风` 完成，不再依赖 ESP32 本地麦克风直采
- 打通桌面代采语音链路，录音输出保持 `16kHz / 16bit / mono PCM`，并直接复用现有 `ASRService`
- 为语音链路补齐交互来源与采集来源标识，可区分 `interaction_surface=device_press / app_text` 与 `capture_source=desktop_mic / device_mic`
- 设备端稳定承担 `listening / transcribing / responding` 状态反馈，并继续作为主交互入口和结果反馈终端

### 前端最小可用性补齐

- 修正前端对语音能力的展示口径，不再把未真实接通的入口误显示为可用
- Chat 页面补齐会话创建、切换与当前会话可视化入口
- Tasks / Events 页面补齐最小可用 CRUD，支持新增、编辑、删除、刷新与错误提示
- Control Center 页面补齐设备控制入口，支持音量、灯光亮度/颜色、唤醒、休眠与静音操作
- Notifications / Reminders 从纯展示调整为可操作列表，至少支持已读、删除与启停

### 后端产品闭环补齐

- 正式采用独立入口承接本地语音客户端，不再让电脑麦克风链路受设备麦克风是否可用影响
- 提醒系统接入真实调度，Reminder 触发后可落到 App 事件流通知
- 明确 `wake_word`、`auto_listen` 当前实现边界，避免在产品口径中继续按已完成能力描述
- 为桌面代采主线补齐异常处理，覆盖设备不可用、ASR 失败与 TTS 失败时的降级反馈

### 联调验收与当前口径

- 新建 `功能讨论区/2026-04-06-P0语音闭环验收记录-按住设备说话+电脑麦克风代采.md`，按新主线重写验收步骤与记录项
- 按端到端链路完成联调检查，覆盖按住触发、设备收音态、服务端收到 PCM、ASR 转文字、处理中反馈、回复生成与最终播报/显示
- 当前稳定输出方式回填为 `设备喇叭 TTS 回放 + 屏幕状态/文字反馈`
- 当前阶段的硬件角色正式收口为：主交互入口、状态反馈终端、显示与在线状态终端、设备命令执行对象；P0 验收结论为 `完成`

---

## 2026-04-08 - macOS 本地联调链路与语音回复修正

### P1 计划文档更新

- 根据当前产品方向，更新 `功能讨论区/TODO/2026-04-08-P1智能待办提醒日历实施计划-macos-app.md`
- 明确 P1 以 macOS 桌面应用为主载体，围绕当前 App 工作台结构规划智能待办、提醒、日历的可视化落地
- 当前计划先聚焦任务/提醒/日历的数据结构、前后端边界、联调顺序与实施拆分，暂不纳入场景模式

### macOS 本地开发与联调

- 更新 `manager.sh`，统一封装 macOS 本地开发常用流程，包括后端启动、前端运行、热重载联调、构建与调试入口
- 保留原有热重载能力，并补充更适合当前桌面联调的快捷操作，方便分别刷新前端、重启后端和查看运行状态
- 为 Flutter macOS 桌面端补齐本地网络访问配置，允许应用连接本机 `127.0.0.1:8765` 后端服务，解决连接页无法接入的问题

### 语音回复链路修正

- 修正设备触摸触发后的回复语言选择逻辑，回复不再被固定为英文
- 支持根据用户输入语言与上下文在中文、英文 TTS 之间切换，中文提问可中文回复，英文提问可英文回复
- 调低默认 TTS 播放音量，降低硬件喇叭播报音量，改善近场使用体验
- 同步调整 `server/services/desktop_voice_service.py` 与 `server/services/tts.py`，使桌面代采音频后的回复链路与当前硬件联调方式保持一致
...
## 2026-04-10 - 首页 Device Snapshot 数据语义补齐与卡片等高整理

### 首页 Device Snapshot 数据语义补齐
